...
<div class="alert alert-danger alert-dismissible fade show" role="alert" id="save-alert">
    <strong>Cannot save invoice!</strong> Invoice was generated from quotation and the amounts should be equal.
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="btn-close"></button>
</div>

<form action="{{route('invoice.update', $invoice->id)}}" method="post" id="invoice-update-form">
@csrf
@method('PATCH')
<input type="hidden" name="account_id" value="{{$account->id}}"/>
<input type="hidden" name="inv_number" id="inv_number" value="{{$invoice->inv_number}}"/>
        ...
        <div class="card-body">
            ...
            <div class="mb-2">
                ...
                <div style="display: flex; justify-content:space-between">
                    <div class="mb-2">
                        ...
                        @if($account->type == 1)<p style="color: #808080">Company VAT Reg: <span> {{$account->vat_registration_number}}</span></p>@endif
                    </div>
                ...
                <div class="row">
                    <div class="mb-3 col-md-3">
                        <label for="exampleInputUsername1" class="form-label">Delivery Date</label>
                        <input value="{{$invoice->delivery_date}}" required type="date" class="form-control" name="delivery_date" id="exampleInputUsername1" autocomplete="off">
                    </div>

                    <div class="mb-3 col-md-3">
                        <label for="exampleInputUsername1" class="form-label">Tax Date</label>
                        <input value="{{$invoice->tax_date}}" required type="date" name="tax_date" class="form-control" id="exampleInputUsername1" autocomplete="off" >
                    </div>

                    <div class="mb-3 col-md-3">
                        <label for="to_date" class="form-label">To</label>
                        <input value="{{$invoice->to_date}}" required type="date" class="form-control" name="to_date" id="to_date" autocomplete="off">
                    </div>

                    <div class="mb-3 col-md-3">
                        <label for="from_date" class="form-label">From</label>
                        <input value="{{$invoice->from_date}}" required type="date" class="form-control" name="from_date" id="from_date" autocomplete="off">
                    </div>
            ...
            <div>
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered" id="dataTableExample">
                        ...
                        <tbody id="table-row">
                            ...
                            <tr>
                                <td>
                                    <input type="text" required name="item_code[]" class="form-control item_code" autocomplete="off" value="{{$item->item_code}}">
                                    <input type="hidden" name="item_id[]" value="{{$item->item_id}}" />
                                </td>
                                <td>
                                    <input type="text" required name="item_description[]" class="form-control item_description" autocomplete="off" value="{{$item->item_description}}">
                                </td>
                                <td>
                                    <input type="text" name="quantity[]" class="form-control quantity" autocomplete="off" value="{{$item->quantity}}">
                                </td>
                                <td>
                                    <input type="text" name="rate[]" class="form-control rate" autocomplete="off" value="{{$item->rate}}">
                                </td>
                                <td>
                                    <input type="text" name="days[]" class="form-control days" autocomplete="off" value="{{$item->days}}">
                                </td>
                                <td>
                                    <input type="text" readonly name="amount[]" class="form-control amount" autocomplete="off"  value="{{$item->amount}}">
                ...
                <div class="mt-2">
                    ...
                    <?php
                        // check if quotation_id is not null
                        $quotationAmt = 0; 
                        $quotation = App\Models\Quotation::find($invoice->quotation_id);
                        if ($quotation) {
                            $quotationAmt = $quotation->total;
                        }
                    ?>
                    <button type="button" class="btn btn-primary" id="save-btn" style="width: 20%">
                        Save Changes
                    </button>
                </div>
            </div>
    ...
    <script defer>
        $(document).ready( function () {
            ...
            function calculateInitialTotal() {
                ...
                let levy = Number(0.02 * vat).toFixed(2);
                vat = vat - levy;
                $('.vat').val(vat);
                $('.levy').val(levy);
                let t = Number(parseFloat(sub_total) + parseFloat(vat) + parseFloat(sub_total_non_taxable) + parseFloat(levy)).toFixed(2);
                $('.total').val(t);
            }
            ...
            $('#save-btn').on('click', (e) => {
                e.preventDefault();
                let quotationAmt = Number("{{$quotationAmt}}");
                if (quotationAmt > 0) {
                    // check if amount is same
                    let invoice_total = Number($('.total').val());
                    if (invoice_total === quotationAmt) {
                        $('#invoice-update-form').submit();
                    } else {
                        $('#save-alert').show();
                        window.scrollTo(0, 0);
                    }
